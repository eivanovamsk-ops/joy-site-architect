...
const EducationSection = () => {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses
      ...
      .sort((a, b) => {
        const isArchivedA = a.id === 23 || a.id === 18;
        const isArchivedB = b.id === 23 || b.id === 18;
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCourses.map((course) => (
                <Link
                  ...
                  className={`bg-card border border-border rounded-2xl overflow-hidden hover:shadow-lg transition-all group block ${(course.id === 23 || course.id === 18) ? "opacity-70" : ""}`}
                >
                  {course.coverImage && (
