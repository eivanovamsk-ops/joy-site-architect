...
import { 
  Calendar, MapPin, Clock, Users, Award, ChevronRight, Share2, CheckCircle2,
  Target, GraduationCap, UserCheck, Lightbulb, HelpCircle, ExternalLink
} from "lucide-react";
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="bg-card rounded-2xl p-6 shadow-lg border border-border">
                ...
                {course.externalUrl ? (
                  ...
                  <CourseEnrollmentForm courseName={course.title} courseDate={course.date} coursePrice={course.price} />
                )}
                ...
                <div key={index} className="bg-card border border-border rounded-xl p-6">
                  ...
                  {lecturer.achievements && lecturer.achievements.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {lecturer.achievements.map((achievement, idx) => (
            ...
            <Tabs defaultValue="day-1" className="w-full">
              <TabsList className="w-full flex-wrap h-auto gap-2 bg-muted/50 p-2 mb-6">
                {course.program.map((day) => (
                  <TabsTrigger key={day.day} value={`day-${day.day}`} className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    День {day.day}
                  </TabsTrigger>
                ))}
              </TabsList>
              {course.program.map((day) => (
                <TabsContent key={day.day} value={`day-${day.day}`} className="bg-card border border-border rounded-xl p-6">
                  ...
                  {day.speaker && <p className="text-primary mb-4">Спикер: {day.speaker}</p>}
          ...
          <div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {relatedCourses.map((relCourse) => (
                <Link key={relCourse.id} to={`/education/course/${relCourse.id}`}
